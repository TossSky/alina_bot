...
class AlinaBot:
    ...
    def _build_time_aware_prompt(self, base_personality: str, time_context: Dict[str, Any]) -> str:
        ...
        time_awareness = (
            ...
            f"ВРЕМЯ СЕЙЧАС: {time_context.get('formatted_time')} ({time_context.get('time_of_day')})"
        )
        ...
        time_awareness += (
            ".\n\n"
            "ВАЖНО О ВРЕМЕНИ:\n"
            "- В истории диалога есть метки с точными датами (например 'вчера (9 октября)'), используй их для точных ответов о датах\n"
            "- Сообщения пользователя содержат временные метки в формате [HH:MM] в начале - это ТОЧНОЕ время когда пользователь написал. Например '[02:46] Привет' означает что он написал в 02:46\n"
            "- Если пользователь спрашивает про время его сообщений - смотри на эти метки [HH:MM] и называй ТОЧНОЕ время из них\n"
            "- Твои собственные сообщения НЕ имеют временных меток, только сообщения пользователя\n"
